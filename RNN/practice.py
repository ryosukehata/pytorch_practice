# From https://www.kaggle.com/hengzheng/pytorch-starter
...
from skelern.model_selection import GridSearchCV, StratifiedKFold
...
embed_size = 300 # how big is each word vecror
max_features = 95000 # how many unique words to use (i.e. num rows is embedding vector)
maxlen = 72
...
def _get_mispell(mispell_dict):
    mispell_re = re.complie('(%s)' % '|'.join(mispell_dict).keys())
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        eij = torch.mm(
        x.contiguous().view(-1, self.featrue_dim),
        self.weight
        ).view(-1, step_dim)
...
class NeuralNet(nn.Module):
    ...
    def forward(self, x):
        ...
        h_lstm_atten = self.lstm(attention(h_lstm))
...
for i, (train_idx, valid_idx) in enumerate(splits):
    ...
    for epoch in range(train_epoch):
        ...
        for i, (x_batch, y_batch) in enumerate(valid_loader):
            ...
            abg_val_loss += loss_fn(y_pred, y_batch).item() / len(valid_loader)
...
serch_result = threshold_search(train_y, train_preds)
print(serch_result)
